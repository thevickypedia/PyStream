...
from models.settings import env
        ...
        if response.ok:
            ...
            for tunnel in tunnels:
                ...
                if hosted := tunnel.get('config', {}).get('addr'):
                    if int(hosted.split(':')[-1]) == env.video_port:
...
def run_tunnel(logger: Logger) -> None:
    ...
    if public_url := get_tunnel(logger=logger):
        logger.info(f"Already hosting {env.video_port} on {public_url}")
        return

    ngrok.set_auth_token(token=env.ngrok_token)
    try:
        endpoint = ngrok.connect(env.video_port, "http", options={"remote_addr": f"{env.video_host}:{env.video_port}"})
    ...
    print(public_url)
    logger.info(f'Tunneling http://{env.video_host}:{env.video_port} through the public URL: {public_url}')
    try:
